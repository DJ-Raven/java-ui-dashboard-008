...
public class Form1 extends javax.swing.JPanel {
    ...
    private void init() {
        ...
        barChart.start();
        lineChart.addLegend("Qty Sold", new Color(12, 84, 175), new Color(0, 108, 247));
        try {
    ...
    private List<ModelChart> getData() throws SQLException {
        ...
        while (r.next()) {
            ...
            data.add(new ModelChart(month, new double[]{total, cost, profit}));
        }
        r.close();
        p.close();
        return data;
    ...
    private List<ModelChart> getDataByItem() throws SQLException {
        List<ModelChart> data = new ArrayList<>();
        String sql = "select ProductName, SUM(QtySold) as Qty from invoice_detail join product using (ProductID) group by ProductID order by Qty desc limit 6";
        PreparedStatement p = DatabaseConnection.getInstance().getConnection().prepareStatement(sql);
        ResultSet r = p.executeQuery();
        while (r.next()) {
